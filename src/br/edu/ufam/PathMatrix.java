...
public class PathMatrix {
	...
	public void buildMatrix() {
		...
		for(int i = 1; i < x.getSize()+1; ++i) {
			for(int j = 1; j < y.getSize()+1; ++j) {
				matrix[i][j] = max(sf(x.tokenSetAt(i-1), y.tokenSetAt(j-1), i, j) + matrix[i-1][j-1],
	...
	public ConsensusPattern merge() throws Exception {
		...
		for (int i = 0; i < x.getSize(); ++i) {
			mc.getPattern().add(TokenSet.tokenSetUnion(x.getPattern().get(i), y.getPattern().get(i)));
		}
		return mc;
	}
	...
	public void print() {
		...
		for(int i = 0; i != matrix.length; ++i) {
			...
			for(int j = 0; j != matrix[i].length; j++) {
				if (j > 0) {
					System.out.printf("%"+(y.tokenSetAt(j-1).getTokensLength()+1)+"d ", matrix[i][j]);
					output.format("%"+(y.tokenSetAt(j-1).getTokensLength()+1)+"d ", matrix[i][j]);
				} else {
