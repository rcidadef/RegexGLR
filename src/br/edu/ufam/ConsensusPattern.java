...
public class ConsensusPattern {
	...
	/**
	 * Gera uma expressão regular generalizada a partir da sequência de
	 * Tokensets que compõem o ConsensusPattern. A generalização se dá da
	 * seguinte forma: 1: Tokenset de um único tipo se torna: Alpha: \w+
	 * Numeric: \d+ Symbol: os próprios símbolos separados por | 2: Com mais de
	 * um tipo: Todos os Tokens separados por | 3: Alinhado com gap Pode ser um
	 * dos casos acima, mas com um ? após o grupo 4: Tokenset de tamanho 1:
		...
		for (TokenSet ts : pattern) {
			...
			if (ts.getSize() == 1) {
				...
				if (ts.getSize() - 1 == 1) {
					...
					// Tamanho de Tokenset maior que limiar, então é necessário
					...
					case 1: { // Caso em que Tokenset tem apenas 1 tipo de Token
						...
						if (ty == Token.ALPHA) {
							...
							regex += "[";
							// Adiciona Tokens de símbolos à expressão
							for (Token t : ts.getSet()) {
					...
					// Não há necessidade de generalizar, apenas se repete cada
					// Token
					regex += "(";
					for (Token t : ts.getSet()) {
						if (t.getType() != Token.GAP) {
			...
			if (ts.getSize() == 1) {
				...
				if (ts.getSet().size() > threshold) {
					...
					case 1: { // Caso em que Tokenset tem apenas 1 tipo de Token
						...
						int ty = ts.getSetType();
						if (ty == Token.ALPHA) {
							regex += "[a-zA-Z]{" + ts.getMinLength() + "," + ts.getMaxLength() + "}";
						} else if (ty == Token.NUMERIC) {
							regex += "[\\d]{" + ts.getMinLength() + "," + ts.getMaxLength() + "}";
						} else { // ty == Token.SYMBOL
							regex += "[";
							// Adiciona Tokens de símbolos à expressão
							for (Token t : ts.getSet()) {
					...
					case 2: { // Caso com 2 tipos de Token distintos
						...
						switch (types) {
						case 3: // ALPHA e NUMERIC
							regex += "[\\w]{" + ts.getMinLength() + "," + ts.getMaxLength() + "}";
							break;
						case 4: // ALPHA e SYMBOL
							regex += "(?:[a-zA-Z]{" + ts.getMinLength() + "," + ts.getMaxLength() + "}|[";
							// Adiciona Tokens de símbolos à expressão
